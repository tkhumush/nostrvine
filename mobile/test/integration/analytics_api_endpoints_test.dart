...
void main() {
  group('Analytics API New Endpoints Integration Tests', () {
    ...
    group('Trending Videos Endpoint', () {
      test('GET /analytics/trending/vines - returns valid trending data', () async {
        ...
        if (vines.isNotEmpty) {
          ...
          Log.info('   Top video: ${firstVine['views']} views, score: ${score.toStringAsFixed(2)}');
        }
      });
      ...
      test('GET /analytics/trending/vines - supports time windows', () async {
        ...
        for (final window in timeWindows) {
          final response = await http.get(
            Uri.parse('$baseUrl/analytics/trending/vines?window=$window&limit=5'),
            headers: {
              'Accept': 'application/json',
              'User-Agent': userAgent,
            },
          ...
          Log.info('✅ Time window $window: ${(data['vines'] as List).length} videos');
        }
      });

      test('GET /analytics/trending/vines - respects limit parameter', () async {
        final limits = [5, 10, 20];
        
        for (final limit in limits) {
          final response = await http.get(
            Uri.parse('$baseUrl/analytics/trending/vines?window=24h&limit=$limit'),
            headers: {
              'Accept': 'application/json',
              'User-Agent': userAgent,
            },
      ...
      test('GET /analytics/trending/vines - performance check', () async {
        ...
        Log.info('✅ Performance: ${stopwatch.elapsedMilliseconds}ms for 100 videos');
      });
    });

    group('Trending Hashtags Endpoint', () {
      test('GET /analytics/trending/hashtags - returns valid hashtag data', () async {
        final response = await http.get(
        ...
        if (hashtags.isNotEmpty) {
          ...
          Log.info('   Top hashtag: #${firstTag['tag']} (${firstTag['views']} views, ${firstTag['videoCount']} videos)');
        }
      });
    });

    group('Top Creators Endpoint', () {
      test('GET /analytics/trending/creators - returns valid creator data', () async {
        final response = await http.get(
        ...
        if (creators.isNotEmpty) {
          ...
          Log.info('   Top creator: ${topCreator['totalViews']} views, ${topCreator['videoCount']} videos');
        }
      });
    });
    ...
    group('Related Videos Endpoint', () {
      test('GET /analytics/vines/{id}/related - returns related videos', () async {
        ...
        if (trendingResponse.statusCode != 200) {
          Log.warning('⚠️ Cannot test related videos - trending endpoint failed');
          return;
        }
        ...
        if (vines.isEmpty) {
          Log.warning('⚠️ Cannot test related videos - no trending videos available');
          return;
        }
        ...
        final hashtagResponse = await http.get(
          Uri.parse('$baseUrl/analytics/vines/$videoId/related?algorithm=hashtag&limit=10'),
          headers: {
            'Accept': 'application/json',
            'User-Agent': userAgent,
          },
        ...
        Log.info('✅ Related videos (hashtag): ${relatedVines.length} videos returned');
        
        // Test co-watch algorithm
        final cowatchResponse = await http.get(
          Uri.parse('$baseUrl/analytics/vines/$videoId/related?algorithm=cowatch&limit=10'),
          headers: {
            'Accept': 'application/json',
            'User-Agent': userAgent,
          },
        ...
        if (cowatchResponse.statusCode == 200) {
          ...
          Log.info('✅ Related videos (co-watch): ${cowatchVines.length} videos returned');
        }
      });
    });
    ...
    group('Error Handling', () {
      test('handles invalid video ID for related videos', () async {
        ...
        Log.info('✅ Invalid video ID handled correctly: ${response.statusCode}');
      });

      test('handles invalid time window parameter', () async {
        final response = await http.get(
          Uri.parse('$baseUrl/analytics/trending/vines?window=invalid&limit=10'),
          headers: {
            'Accept': 'application/json',
            'User-Agent': userAgent,
          },
    ...
    group('Caching and Performance', () {
      ...
      test('concurrent requests performance', () async {
        ...
        Log.info('✅ Concurrent requests (5): ${stopwatch.elapsedMilliseconds}ms total');
        Log.info('   Average per request: ${(stopwatch.elapsedMilliseconds / 5).toStringAsFixed(0)}ms');
      });
    });
  });
}